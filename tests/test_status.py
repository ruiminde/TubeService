...
class TestLineStatus(TestCase):
    ...
    def test_get_latest_station_problems3(self):
        response_mock = MagicMock()
        response_mock.status_code = 200
        response_mock.text = _RESPONSE_BODY_PROBLEMS_STATION3
        self.metro_lisboa._http_connector.get.return_value = response_mock
        ...
        expected = {
            LineStatus.LINE_RED: (LineStatus.STATUS_OK, None),
            LineStatus.LINE_YELLOW: (LineStatus.STATUS_OK, None),
            LineStatus.LINE_BLUE: (LineStatus.STATUS_HALT, LineStatus.REASON_HALT),
            LineStatus.LINE_GREEN: (LineStatus.STATUS_OK, None),
        }

        self.assertDictEqual(expected, actual)

    def test_get_latest_station_problems4(self):
        response_mock = MagicMock()
        response_mock.status_code = 200
        response_mock.text = _RESPONSE_BODY_PROBLEMS_STATION4
        self.metro_lisboa._http_connector.get.return_value = response_mock
    ...
    def test_get_latest_station_problems5(self):
        response_mock = MagicMock()
        response_mock.status_code = 200
        response_mock.text = _RESPONSE_BODY_PROBLEMS_STATION5
        self.metro_lisboa._http_connector.get.return_value = response_mock
